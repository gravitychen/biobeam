...
# # Bessel Lattice Lightsheet PSF 可视化
# 
# 本 notebook 用于可视化 bessel lattice lightsheet 的：
# 1. **Excitation PSF** - 激发点扩散函数（使用 Bessel lattice）
...
units = (0.1, 0.1, 0.1)  # (dx, dy, dz) 单位：微米 micrometers
...
print(f"激发 (Bessel Lattice): λ={lam_excitation} μm, NA1={NA1_excitation}, NA2={NA2_excitation}, σ={sigma_excitation}, kpoints={kpoints_excitation}")
print(f"检测: λ={lam_detection} μm, NA={NA_detection}")


# ## 1. 计算 Excitation PSF (Bessel Lattice)
# 

# In[33]:


print("正在计算 Excitation PSF (Bessel Lattice)...")
psf_excitation = focus_field_lattice(
    shape=shape,
    units=units,
    lam=lam_excitation,
    NA1=NA1_excitation,
    NA2=NA2_excitation,
    sigma=sigma_excitation,
...
# 对 Bessel Lattice Beam 进行 dither 处理，生成 lightsheet
# Dither 处理：在 X 方向上密集扫描多个位置并求和，将 bessel beam 转换为 lightsheet
print("正在对 Bessel Lattice Beam 进行 dither 处理以生成 lightsheet...")

# 使用更密集的扫描步长：使用 lam/max_NA 或更小的步长
max_NA = max(NA1_excitation, NA2_excitation)
# 使用更小的步长，使扫描更密集
dx_parallel = lam_excitation / max_NA  # 原来用 2*lam，现在用 lam，更密集
print(f"理论并行光束距离 dx_parallel = {dx_parallel:.4f} μm")

# 计算扫描步数（以像素为单位）
dx_parallel_pixels = dx_parallel / units[0]
# 使用更小的步长，至少 1 个像素，但尽量使用更小的物理步长
# 如果计算出的步长小于 1 像素，则使用 1 像素；否则使用计算出的步长
ind_step_pixels = max(1, int(np.round(dx_parallel_pixels)))
# 为了更密集，我们可以使用更小的步长，比如使用 0.5 倍或更小
# 这里我们使用更密集的扫描：步长为 1-2 个像素
ind_step = max(1, min(2, ind_step_pixels))  # 使用 1-2 个像素的步长

Nx = shape[0]
# 扫描整个 X 方向，使用密集的步长
print(f"密集扫描步长: {ind_step} 像素 ({ind_step * units[0]:.4f} μm)")

# 对每个扫描位置进行平移并求和
# 扫描整个 X 方向，步长为 ind_step
psf_excitation_lightsheet = np.zeros_like(psf_excitation)
scan_positions = list(range(0, Nx, ind_step))
print(f"扫描位置数: {len(scan_positions)} 个位置（覆盖整个 X 方向）")

for i in scan_positions:
    # 在 X 方向（axis 2）上平移
    psf_shifted = np.roll(psf_excitation, i, axis=2)
    psf_excitation_lightsheet += psf_shifted

# 归一化（除以扫描位置数，保持能量一致）
psf_excitation_lightsheet = psf_excitation_lightsheet / len(scan_positions)

print(f"Dither 处理后 Excitation PSF (lightsheet) 形状: {psf_excitation_lightsheet.shape}")
print(f"Dither 处理后 Excitation PSF 最大值: {psf_excitation_lightsheet.max():.4f}")
print(f"Dither 处理后 Excitation PSF 最小值: {psf_excitation_lightsheet.min():.4f}")

# 使用 dither 处理后的 lightsheet 作为 excitation PSF
psf_excitation = psf_excitation_lightsheet
...
# - 方法：交换 X 和 Y 轴，即转置 (0, 2, 1) 或使用 np.swapaxes
...
# 生成 3x2 组合可视化
print("正在生成 PSF 3x2 组合可视化...")
print(f"NA1_exc={NA1_excitation}, NA2_exc={NA2_excitation}, σ={sigma_excitation}, kpoints={kpoints_excitation}, NA_det={NA_detection}")

fig = visualize_all_psf_combined(
    psf_excitation,
    psf_detection,
    psf_effective,
    title=f'Bessel Lattice PSF 组合可视化<br>NA1={NA1_excitation}, NA2={NA2_excitation}, σ={sigma_excitation}, kpoints={kpoints_excitation}, NA_det={NA_detection}',
    isomin=None,
    opacity=0.05,
    surface_count=21,
