...
def compute_biobeam_bridge_outputs(
    ...
    illumination = propagate_complex_pupil_to_volume(
    ...
    detection = make_gaussian_detection_psf(
        illumination.shape,
        llspy_like["x_um"],
        z_um,
...
def save_bridge_outputs(output_root: Path, bridge: Dict[str, Any]) -> Path:
    ...
    np.savez_compressed(
        ...
        detection_psf_gaussian_cpu=bridge["detection_psf_gaussian_cpu"],
    ...
    (outdir / "README__what_is_inside.md").write_text(
        "\n".join(
            [
                ...
                "- `illumination_volume_cpu`: 从 complex pupil field 做 CPU 角谱传播得到的 illumination volume。",
                "- `detection_psf_gaussian_cpu`: 当前无 GPU/OpenCL 时使用的检测 PSF 高斯近似。",
                "- `effective_psf_cpu`: `illumination_volume_cpu * detection_psf_gaussian_cpu`。",
            ]
        ),
        encoding="utf-8",
    )
